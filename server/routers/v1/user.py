...
async def create_user(user: UserCreate, request: Request, session: SessionDep) -> User:
    ...
    user_logger = logger.bind(device_type=user_agent, py_version=python_version, fastapi_version=fastapi_version, environment="production", service="writestack", http_scheme=request.url.scheme, route=request.url.path, method=request.method)
    
    user_logger.debug("user.creation.validation.started")
    new_user = User.model_validate(user)
    user_logger.debug("user.creation.validation.success")
    user_logger.info("user.creation.database_commit.started", status_code=201)
    session.add(new_user)
    user_logger.info("user.creation.database_commit.success", user_id=new_user.id)
    session.commit()
    user_logger.info("user.creation.success")
...
async def get_user(user_uuid: uuid.UUID, request: Request, session: SessionDep) -> User:
    user_agent = request.headers.get('User-Agent')
    user_logger = logger.bind(device_type=user_agent, py_version=python_version, fastapi_version=fastapi_version, environment="production", service="writestack", http_scheme=request.url.scheme, route=request.url.path, method=request.method, user_id=str(user_uuid))
    
    user_logger.debug("user.search.started")
    user = session.exec(select(User).where(User.id == user_uuid)).first()

    if not user:
        user_logger.exception("user.search.failed", detail="User not found", status_code=404)
        raise HTTPException(status_code=404, detail='User not found')
    
    user_logger.info("user.search.success", status_code=200)
    return user
...
async def update_user(user_uuid: uuid.UUID, updated_user: UserUpdate, request: Request, session: SessionDep) -> User:
    ...
    user_logger.info("user.update.database_commit.started", status_code=200)
    session.add(user)
    user_logger.info("user.update.database_commit.complete")
    session.commit()
    user_logger.info("user.update.success")
...
async def get_newsletters_for_users(user_uuid: uuid.UUID, request: Request, session: SessionDep) -> List[Newsletter]:
    ...
    if not newsletters:
        user_logger.exception("newsletter.search.failed", detail="User not found", status_code=404)
        raise HTTPException(status_code=404, detail='No newsletters found')

    user_logger.info("newsletter.search.success", status_code=200)
    return newsletters
