...
async def get_post(post_uuid: uuid.UUID, request: Request, session: SessionDep) -> Post:
    ...
    post_logger.info("post.search.success", detail="Post found", status_code=200)
    return post
...
async def create_post(post: PostCreate, request: Request, session: SessionDep) -> Post:
    ...
    post_logger.info("post.creation.success", detail="Post created", status_code=201, post_id=str(create_post.id))
    return create_post
